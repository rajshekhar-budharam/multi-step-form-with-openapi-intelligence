...
export default function Step2({ value, onChange, lang = 'en', errors = {}, showErrors = false }) {
  ...
  return (
    <section aria-labelledby="step2-title">
      <h2 id="step2-title" className="text-2xl font-bold mb-4 pb-3 border-b-2 border-indigo-600 text-gray-900">{t(lang, 'step2')}</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
        ...
        <label>
          ...
          <select className="w-full px-2 py-1 border rounded" value={value.employmentStatus} onChange={e => handle('employmentStatus', e.target.value)} onBlur={() => setTouched(prev => ({ ...prev, employmentStatus: true }))}>
            <option value="">{t(lang, 'select')}</option>
        ...
        <label>
          ...
          <input type="number" min="0" className="w-full px-2 py-1 border rounded" value={value.monthlyIncome} onChange={e => handle('monthlyIncome', e.target.value)} onBlur={() => setTouched(prev => ({ ...prev, monthlyIncome: true }))} />
