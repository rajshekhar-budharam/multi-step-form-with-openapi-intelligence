import { GROQ_URL, MODEL_LLAMA } from './constants';

export async function generateSuggestion(prompt, { apiKey, timeout = 8000, model = MODEL_LLAMA } = {}) {
  ...
  try {
    const res = await fetch(GROQ_URL, {
      ...
      body: JSON.stringify({
        model,
        messages: [
          { role: 'system', content: 'You are a helpful, empathetic assistant that writes concise paragraphs for applications or forms.' },
...
function localFallbackSuggestion(prompt) {
  ...
  try {
    const applicantMatch = prompt.match(/Applicant data:\n([\s\S]*?)\n\nCurrent field value:/i)
    const exampleMatch = prompt.match(/Instruction \/ example for tone and intent:\n"([\s\S]*?)"/i)
    const currentMatch = prompt.match(/Current field value:\n([\s\S]*?)\n\nInstruction/i)
    ...
    if (applicantRaw) {
      ...
      const parts = applicantRaw.split(/\r?\n/).map(l => l.trim()).filter(Boolean)
      const phrases = parts.map(p => p.replace(/:\s*/, ' is '))
      summary = phrases.join(', ')
    }

    const intro = exampleInstruction || 'I am writing to explain my financial circumstances.'
    const bodyParts = []
    if (summary) bodyParts.push(`Applicant details: ${summary}.`)
    if (currentField && currentField !== '(empty)') bodyParts.push(`Current detail: ${currentField}.`)
    bodyParts.push(intro)

    // A simple empathetic paragraph constructed locally
    const paragraph = `${bodyParts.join(' ')} I am currently experiencing financial hardship and would appreciate consideration of my application. Please consider this statement as a truthful summary of my situation.`
...
export function mockSubmit(data, { delay = 1000 } = {}) {
  return new Promise((resolve) => setTimeout(() => resolve({ ok: true, data }), delay))
}
